...
_GitRepoInfo = provider(
    ...
    fields = {
        ...
        "remote_url": "URL of the git repository to fetch from",
        "commit": "Specific commit to be checked out",
        "branch": "Specify a branch to fetch HEAD references",
        "shallow_since": "Actual date and time of the commit of the checked out data.",
        "update_shallow": "Accept refs that require updating .git/shallow",
    },
)
...
_RuleExecInfo = provider(
    ...
    fields = {
        "fetch_retries_count": "Amount of retries attempts for fetching a repository before failing",
        "fetch_retry_timeout_in_sec": "Time to wait for a single fetch retry attempt",
    },
)
...
def checkout(ctx, cache_dir):
    ...
    if result.success == False:
        _log(
            ctx,
            "Failed to reset commit hash. name: {}, value: {}".format(
                ctx.name,
                ctx.attr.commit,
            ),
        )
        ...
        _log(
            ctx,
            "Successfully reset commit hash. name: {}, value: {}".format(
                ctx.name,
                ctx.attr.commit,
            ),
        )
...
def fetch(ctx, cache_dir, should_update_shallow = False):
    ...
    git_repo_info = _create_git_repo_info(
        ctx,
        cache_dir,
        should_update_shallow = should_update_shallow,
    )
    rule_exec_info = _create_rule_exec_info(ctx)

    _log(
        ctx,
        "Fetching repository from branch. name: {}, flags: {}".format(
            ctx.name,
            git_repo_info.shallow_since,
        ),
    )
...
def clone(ctx, cache_dir):
    ...
    _log(
        ctx,
        "Cloning repository. name: {}, path: {}, flags: {}".format(
            ctx.name,
            cache_dir,
            git_repo_info.shallow_since,
        ),
...
def _execute_with_retry(
    ...
    for counter in range(retry_count):
        _log(ctx, ("Attempt #{}..." if counter == 0 else "Retry attempt #{}...").format(counter + 1))
        result = ctx.execute(
            args,
